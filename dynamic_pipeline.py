...
from pyspark.sql.functions import col, to_date, regexp_replace, when, lit, trim
...
def add_error_column(df):
    ...
    return df.withColumn(
        ...
        .when(col("account_no").isNull(), "Missing Account")
        .when(col("withdrawal_amt_clean").isNull() & col("withdrawal_amt").isNotNull(), "Invalid Withdrawal Amt")
        .when(col("deposit_amt_clean").isNull() & col("deposit_amt").isNotNull(), "Invalid Deposit Amt")
        .otherwise(None)
    )
